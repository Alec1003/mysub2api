<template>
  <!-- Row 1: Core Stats -->
  <div data-ui="brutal-dashboard-stats" class="brutal-dashboard-stats grid grid-cols-2 gap-4 lg:grid-cols-4">
    <!-- Balance -->
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">{{ formatPoints(balance) }} {{ t('common.points') }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="brutal-icon-box bg-blue-100 p-2 dark:bg-blue-900/30">
          <Icon name="dollar" size="md" class="text-blue-600 dark:text-blue-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</p>
          <p class="text-xl font-bold text-gray-900 dark:text-white">
            <span class="text-blue-600 dark:text-blue-400" :title="t('dashboard.actual')">{{ formatPoints(stats?.today_actual_cost) }} {{ t('common.points') }}</span>
            <span class="text-sm font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ formatPoints(stats?.today_cost) }} {{ t('common.points') }}</span>
          </p>
          <p class="text-xs">
            <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
            <span class="text-blue-600 dark:text-blue-400" :title="t('dashboard.actual')">{{ formatPoints(stats?.total_actual_cost) }} {{ t('common.points') }}</span>
            <span class="text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ formatPoints(stats?.total_cost) }} {{ t('common.points') }}</span>
          </p>
        </div>
      </div>
    </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-blue-600 dark:text-blue-400" :title="t('dashboard.actual')">
            {{ formatPoints(item.total_actual_cost) }} {{ t('common.points') }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono text-gray-900 dark:text-white">{{ formatPoints(item.today_actual_cost) }} {{ t('common.points') }}</span>
          </div>
          <div class="flex items-center justify-between">
              ...
              <template v-else>
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="font-mono text-gray-700 dark:text-gray-200">
                    {{ formatPoints((quotaVal(item.quota, `${w}_usage_usd`) as number) ?? 0) }} / {{ formatPoints(quotaVal(item.quota, `${w}_limit_usd`) as number) }} {{ t('common.points') }}
                  </span>
                </div>
                <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
                  <div
